...
public class HelloController {
    ...
    public static boolean isBeforeCurrentHour(LocalDateTime dateTime) {
        LocalDateTime currentDateTime = LocalDateTime.now();
        return dateTime.isBefore(currentDateTime);
    }

    public static boolean isMoreThanAWeekAhead(LocalDate inputDate) {
        LocalDate now = LocalDate.now();
        Period period = Period.between(now, inputDate);
        return period.getDays() > 7 || period.getMonths() > 0 || period.getYears() > 0;
    }
